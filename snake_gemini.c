...
#define MACAS_POR_NIVEL 15
#define NIVEL_MAX 5
#define MAX_OBSTACULOS 200
...
static const uint8_t FONT_BITMAP[FONT_CHAR_COUNT][FONT_HEIGHT] = {
    ...
    // Caractere: '0' (ASCII 48) - NOVO
    {0x3E, 0x41, 0x41, 0x41, 0x41, 0x41, 0x3E},

    // Caractere: '1' (ASCII 49) - NOVO
    {0x00, 0x04, 0x06, 0x04, 0x04, 0x04, 0x0E},

    // Caractere: '2' (ASCII 50) - NOVO
    {0x3E, 0x41, 0x40, 0x1E, 0x01, 0x41, 0x7F},

    // Caractere: '3' (ASCII 51) - NOVO
    {0x7E, 0x40, 0x40, 0x3C, 0x40, 0x40, 0x7E},

    // Caractere: '4' (ASCII 52) - NOVO
    {0x10, 0x10, 0x10, 0x10, 0x7F, 0x10, 0x10}, // Simplificado, pode ser 0x08,0x18,0x28,0x7F,0x08,0x08

    // Caractere: '5' (ASCII 53) - NOVO
    {0x7F, 0x01, 0x01, 0x3F, 0x40, 0x40, 0x3F},

    // Caractere: '6' (ASCII 54) - NOVO
    {0x3E, 0x41, 0x01, 0x3F, 0x41, 0x41, 0x3E},

    // Caractere: '7' (ASCII 55) - NOVO
    {0x7F, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40},

    // Caractere: '8' (ASCII 56) - NOVO
    {0x3E, 0x41, 0x41, 0x3E, 0x41, 0x41, 0x3E},

    // Caractere: '9' (ASCII 57) - NOVO
    {0x3E, 0x41, 0x41, 0x3F, 0x01, 0x01, 0x3E},
...
void limpar_cauda(node* lista, int x, int y) {
    node* temp = lista;
    int dy = 0;
    while (dy < SIZE_SNAKE) {
        int dx = 0;
        while (dx < SIZE_SNAKE) {
            set_pixel(y + dy, temp->next->x + dx, BLACK);
            dx++;
        }
        dy++;
    }
...
void desenha_snake(node* lista) {
    int i = 0;
    while (lista) {
        uint16_t color = (i == 0) ? WHITE : CORAL; // Cabeça branca, corpo coral
...
void mover_snake(node** lista, int dx, int dy, int* pontos) {
    ...
    *lista = novo; // Atualiza a cabeça da lista
    ...
    if (novo_x == apple_x && novo_y == apple_y) {
        *pontos += valor_maca;
    ...
    limpar_cauda(*lista, cabeca->x, cabeca->y);
}
...
void gerar_obstaculos() {
    ...
    int porcentagem = (nivel - 1) * 5; // Aumenta a porcentagem de obstáculos com o nível
...
void desenha_obstaculos() {
    for (int i = 0; i < qtd_obstaculos; i++) {
        for (int dy = 0; dy < SIZE_SNAKE; dy++) {
            for (int dx = 0; dx < SIZE_SNAKE; dx++) {
...
int main() {
    ...
    while (1) {
        ...
        while (1) {
            ...
            // Assumindo que os botões 0, 1, 2, 3 correspondem a:
            // BTN0 (0x01): Direita (originalmente 3)
            // BTN1 (0x02): Esquerda (originalmente 4)
            // BTN2 (0x04): Baixo (originalmente 2)
            // BTN3 (0x08): Cima (originalmente 1)
            // A sua atribuição de botões parece invertida para cima/baixo/esquerda/direita.
            // Vou manter a sua lógica original para as direções, mas esteja ciente.
            // Para joystick/D-pad:
            ...
            // Mantendo a sua lógica original de mapeamento de botões:
            if (btns & 0x01) direcao = 3; // BTN0 -> Direita
            else if (btns & 0x02) direcao = 4; // BTN1 -> Esquerda
            else if (btns & 0x04) direcao = 2; // BTN2 -> Baixo
            else if (btns & 0x08) direcao = 1; // BTN3 -> Cima
            

            // Move a cobra na direção atual
            mover_snake(&lista, 0, 0, &pontos); // dx, dy são determinados pelo switch-case abaixo,
                                                // mas a função mover_snake não usa dx, dy diretamente
                                                // na sua assinatura. Ela já está usando o novo_x/novo_y.
                                                // O `dx` e `dy` passados aqui estão sendo ignorados.
                                                // PRECISA ser corrigido para passar os dx, dy corretos.
            ...
            mover_snake(&lista, current_dx, current_dy, &pontos);
            ...
            if ((pontos / valor_maca) >= (nivel * MACAS_POR_NIVEL) && nivel < NIVEL_MAX) {
                nivel++;
