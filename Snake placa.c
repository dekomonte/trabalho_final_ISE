...
#define _BSD_SOURCE //Vai evitar declaração implicita do usleep()
...
void init_peripherals() {
    ...
    // Mapeia a memória do bridge leve (LW) no espaço do usuário
    volatile void *lw_virtual = mmap(NULL, LW_BRIDGE_SPAN, PROT_READ | PROT_WRITE, MAP_SHARED, fd, LW_BRIDGE_BASE);
    if (lw_virtual == MAP_FAILED) {
        perror("Erro ao mapear memória");
        exit(1);
    }

    // Configura os ponteiros para os periféricos, corrigindo PERIPHERAL_BASE para lw_virtual
    leds = (volatile uint32_t *)(lw_virtual + DEVICES_LEDS);
    hex_display1 = (volatile uint32_t *)(lw_virtual + DEVICES_HEX_DISPLAY_01);
    hex_display2 = (volatile uint32_t *)(lw_virtual + DEVICES_HEX_DISPLAY_02);
    buttons = (volatile uint32_t *)(lw_virtual + DEVICES_BUTTONS);
    switches = (volatile uint32_t *)(lw_virtual + DEVICES_SWITCHES);
