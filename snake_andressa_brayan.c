...
void gerar_obstaculos() {
    ...
    int porcentagem = (nivel - 1) * 3; // Aumenta a porcentagem de obstáculos com o nível
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (maca_get >= MACAS_POR_NIVEL && nivel < NIVEL_MAX) {
                ...
                lista = NULL;
                dx = 0; dy = SIZE_SNAKE; // Reinicia a direção para baixo
                maca_get = 0;
                flag_fim_de_jogo = 0;
                ...
                gerar_obstaculos();       // Gera novos obstáculos
                subir_nivel();            // Exibe a tela de subida de nível
                muda_cor_fundo(BLACK);    // Limpa a tela novamente após a mensagem
                continue;
            }
